...
Shipit.prototype.initSshPool = function () {
  ...
  var serverArray = _.isArray(this.config.servers) ? this.config.servers : [this.config.servers];

  const servers = serverArray.reduce(function(servers, server) {

    if(_.isString(server) && server.match(/\S*@\S*/)) {
  ...
  this.pools = Object.keys(servers).reduce(function(pools, role) {

    pools[role] = new sshPool.ConnectionPool(servers[role], _.extend({}, this.options, _.pick(this.config, 'key', 'strict')));
...
Shipit.prototype.remote = function (command, options, cb) {
  ...
  if (options && options.role) {
    const pool = this.pools[options.role]
    delete options.role;
    return pool.run(command, options, cb);
  }

  return this.pools.all.run(command, options, cb);
...
Shipit.prototype.remoteCopy = function (src, dest, options, callback) {
  ...
  return this.pools[role].copy(src, dest, options, callback)
};
